...
import 'package:android_cache_cleaner/domain/usecases/clean_cache_usecase.dart';
...
class CleaningBloc extends Bloc<CleaningEvent, CleaningState> {
  ...
  CleaningBloc(this._cleanCacheUseCase, this._cleaningRepository)
  ...
  Future<void> _onStartCleaning(
    ...
    try {
      ...
      final isAccessibilityEnabled = await _cleaningRepository
      ...
      if (!isAccessibilityEnabled) {
        ...
        final success = await _cleaningRepository.triggerAccessibilityCleaning(
          event.packageNames,
        );
        if (success) {
          emit(CleaningSuccess());
          ...
          emit(
            const CleaningError(
              "Failed to start accessibility service automation.",
            ),
          );
        }
      }
